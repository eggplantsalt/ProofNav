...
- M2.1 is frozen complete under the conditional claims in `docs/M2_ARCHITECTURE.md`. M3-A has a default-off real DUET hook, opaque agent-visible episode lineage, an entity-SUPPORT-only successor, and an exact artifact/signal/prefix registry. Its bounded seen-scan artifact has descriptive error `2/6` (`confidence=null`), so engineering M3-A is complete only for the fixed recorded slice while the scientific gate is `Revise`: strict `alpha_F=0.05` remains `UNRESOLVED`. Attribute/relation/room/coverage/identity, live producer attestation, M3-B, M4/M5, training, formal paired data and further experiments require explicit user authorization.
